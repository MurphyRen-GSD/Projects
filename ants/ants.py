...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.is_container == True and insect.is_container == False and not self.ant.contained_ant:
                    self.ant.contain_ant(insect)
                elif self.ant.is_container == False and insect.is_container == True:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.is_queen:
                return
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if self.scare_once == False and self.direction == 'back':
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        actual_bee_place = self.place
        count_space = 0
        while actual_bee_place.entrance != None or actual_bee_place != hive:
            if len(actual_bee_place.bees) > 0 and self.min_range <= count_space <= self.max_range:
                return random_or_none(actual_bee_place.bees)
            count_space += 1
            
            actual_bee_place = actual_bee_place.entrance
        return None
        # END Problem 3 and 4
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.place.bees != None:
        	target_bees = list(self.place.bees)
        for target_bee_element in target_bees:
        	Insect.reduce_armor(target_bee_element, self.damage)
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if self.is_container == True and other.is_container == False:
            return True
        # END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_watersafe:
            insect.armor = 0
            Place.remove_insect(self, insect)
        # END Problem 11
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        def been_buffed(buff_ant):
            buff_ant.damage = 2*buff_ant.damage
            self.pumped_ant.append(buff_ant)
        ...
        if not self.is_queen:
            ...
            while temp:
                if temp.ant:
                    if temp.ant.is_container and temp.ant.contained_ant and temp.ant.contained_ant not in self.pumped_ant:
                        been_buffed(temp.ant.contained_ant)
                    elif temp.ant not in self.pumped_ant:
                        been_buffed(temp.ant)
                temp = temp.exit
        # END Problem 13
...
def make_scare(action, bee):
    ...
    def new_scare_action(colony):
    	if bee.scare_once == False:
    		bee.scare_once = True
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    """
    def new_apply_effect(colony):
        nonlocal duration
        if duration and effect == make_slow:
            return effect(bee.action)(colony)
            duration -= 1
        if duration and effect == make_scare:
            return effect(bee.action)(colony)
            duration -= 1
        else:
            bee.action
    ...
    if hasattr(effect, 'make_scare'):
    	duration = bee.scary_duration
    else:
    	duration = bee.slow_duration
