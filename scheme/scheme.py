...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        len_f, len_v = len(formals), len(vals)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    check_form(bindings, 0)

    bindings.map(lambda item: check_form(item, 2, 2))
    formals = bindings.map(lambda item: item.first)
    vals = bindings.map(lambda item: scheme_eval(item.second.first, env))
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if (scheme_symbolp(first) and first in SPECIAL_FORMS):
                ...
                expr_part = optimized_eval(first,env)
                check_procedure(expr_part)
                args = rest.map(lambda func: optimized_eval(func,env))
